...
/**
 * -is_onlineプロパティとchatterMapを更新する
 */
...
/**
 * -is_onlineプロパティとchatterMapを更新すうｒ
 */
    ...
    io
      ...
      .on('connection', function (socket){
        ...
        socket.on('disconnect', function () {
          console.log('** user %s closed browser windows or tab **', socket.user_id);
          signOut(io, socket.user_id);
        });
        ...
        socket.on('updateavatar', function (avtr_map) {
          crud.update(
            'user',
            {'_id': makeMongoId(avtr_map.person_map)},
            {css_map: avtr_map.css_map},
            function (result_list) { emitUserList(io); }
          );
        });
